...
class RepositoryConfig:
    ...
    def from_mapping(
        ...
        if not isinstance(mapping, Mapping):
            raise ValueError("Repository configuration must be a mapping/object")
        ...
        if not isinstance(iotdb, Mapping):
            raise ValueError("iotdb must be a mapping/object")

        return cls(backend, metadata, signals, max_points, dict(iotdb))

    @classmethod
    def from_file(cls, path: str | Path) -> "RepositoryConfig":
        ...
        if not isinstance(data, Mapping):
            raise ValueError(
                f"Configuration file must contain a top-level mapping/object: {path}"
            )
